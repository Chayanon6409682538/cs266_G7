...
//สร้างตาราง ระบุจำนวนระเบิด setemoji 
...
Game.prototype.restart = function() {
  // ในฟังก์ชัน restart คุณสามารถใส่โค้ดที่เกี่ยวข้องกับการ restart เกม
};
Game.prototype.init = function () {
  this.prepareEmoji()
//ระบุจำนวนช่องที่เป็นไปได้
...
//คำนวณช่องที่ใกล้ระเบิดและแสดงเป็นค่าตัวเลขเพื่อคำนวณ
...
Game.prototype.startTimer = function () {
  if (this.timer) return; // ถ้านับถอยหลังอยู่แล้ว ไม่ต้องทำอะไร
  this.remainingTime = 180; // กำหนดเวลาเริ่มต้นใหม่ทุกครั้งที่เริ่มเกม
  this.timer = setInterval(function () {
    if (this.remainingTime <= 0) {
      clearInterval(this.timer);
      this.timer = null;
      // Perform actions when the countdown reaches zero (e.g., game over)
      // Example: this.gameOver();
    } else {
      this.remainingTime--;
...
Game.prototype.startCountdown = function (seconds) {
    ...
    this.timer = setInterval(function () {
      if (this.remainingTime <= 0) {
        clearInterval(this.timer);
        this.timer = null;
        this.result = 'lost'; // Set the result to 'lost' when the countdown reaches zero
        this.showMessage();
      } else {
        this.remainingTime--;
        this.updateTimerDisplay();
...
Game.prototype.startTimer = function (duration) {
    if (this.timer) return;
    this.startCountdown(duration); // Start a countdown with the specified duration
}
